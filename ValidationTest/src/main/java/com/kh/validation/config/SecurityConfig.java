...
//해당 클래스가 설정파일임을 인지(개발자 입장에서) + 객체 생성
//security 설정을 위해서는 WebSecurityConfigurerAdapter
//안에 정의된 메소드를 오버라이딩해야 한다.
@Configuration
//설정 파일 활성화
@EnableWebSecurity
public class SecurityConfig extends WebSecurityConfigurerAdapter{
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests() //요청 url에 따른 인정 처리 설정
			.antMatchers("/"
		...
		http.formLogin() // 시큐리티 로그인 기능 정의 시작
			.loginPage("/member/loginForm") // 로그인 폼 url을 작성
			...
			.permitAll();//위에서 작성한 url은 아무나 접근 가능
	}
